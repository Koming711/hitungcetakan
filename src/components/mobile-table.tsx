...
interface MobileTableProps<T> {
  ...
  showAsButtons?: boolean // New prop to show actions as buttons instead of dropdown
}
            ...
            <tr>
              {columns.map((col) => (
                <th
                  key={col.key}
                  className={cn(
                    "px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase whitespace-nowrap",
                    col.className
                  )}
                >
                  {col.title}
              ...
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-600 uppercase whitespace-nowrap">
                Aksi
              </th>
            </tr>
          </thead>
        ...
        {data.map((item, idx) => {
          ...
          return (
            <div
              ...
              onClick={() => isMobile && toggleSelection(item)}
              className={cn(
                "bg-white border border-slate-200 rounded-lg p-4 cursor-pointer transition-all",
